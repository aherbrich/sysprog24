...
void employee_thread(){
    ...
    // following the logic of the lectures, the cond is set to true         
    unlock(nr_of_doener_mutex); 
}

void customer_thread(){
    lock(only_one_customer_at_a_time_mutex);
    // we need to lock this whole routine, because:
    // imagine we have 2 customers and 2 employees
    // 1. both customers, enter the store but there are no doeners
    // 2. both customers will wait (on some cond)
    // 3. now BOTH employees finish their doener and signal the cond
    //    (following the logic of the lectures, the cond is set to true
    //     and then true again by the second signal)
    // 4. now the first customer wakes up from wait() and sets the cond to false
    // 5. the second customer will not wake up, because the cond is false again 
    // -> two doeners can not be represented by the binary true/false of a cond
